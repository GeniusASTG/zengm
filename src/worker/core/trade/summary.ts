...
const summary = async (teams: TradeTeams): Promise<TradeSummary> => {
	...
	const softCapCondition =
		!g.get("hardCap") &&
		((ratios[0] > g.get("tradeMatchingPercentage") && overCap[0]) || (ratios[1] > g.get("tradeMatchingPercentage") && overCap[1]));
	...
	if (softCapCondition) {
		...
		s.warning = `The ${s.teams[j].name} are over the salary cap, so the players it receives must have a combined salary of less than ${g.get("tradeMatchingPercentage")}% of the salaries of the players it trades away.  Currently, that value is ${ratios[j]}%.`;
